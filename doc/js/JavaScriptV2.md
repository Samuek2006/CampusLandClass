...
![](https://i.ibb.co/gMRq0j2f/image.png)
...
```js
// Versión externa: separa responsabilidades y favorece caché del navegador.
console.log("¡Hola, JS! (externo)");
```
...
Muestra un mensaje o valor en la consola, siendo el método más común para depuración.
...
Muestra una advertencia en la consola, útil para señalar posibles problemas.
...
Muestra un mensaje informativo. Similar a `console.log`, pero con un propósito más específico.
...
Muestra datos en formato de tabla, lo cual es útil para visualizar arrays y objetos de manera organizada.
...
Limpia la consola. Ideal para eliminar mensajes y comenzar de nuevo con una consola vacía.
...
Agrupan mensajes en un bloque que se puede expandir o contraer, lo cual ayuda a organizar mejor los mensajes en la consola.
...
Miden el tiempo que toma ejecutar una sección de código, útil para analizar el rendimiento.
...
2. Descargando manualmente los archivos.
   ...
   <img src="https://i.ibb.co/MM9N3zb/developer-13-11-2024-11-53-264.gif"  />
...
La desestructuración en JavaScript es una característica que permite descomponer o extraer valores de arreglos y objetos de manera más concisa. Con la desestructuración, se pu asignar valores a variables de una manera más eficiente y legible. La sintaxis de la desestructuración utiliza corchetes **{}** para objetos y corchetes cuadrados **[]** para arreglos.
...
```javascript
// Cadena que el usuario escribe en un input
const raw = "JavaScript, css, HTML, CSS, javascript, Node  ,  html";

// 1) Normalizamos: separamos por coma, quitamos espacios y pasamos a minúsculas
const tagsNormalizadas = raw
  .split(",")
  .map(t => t.trim().toLowerCase())
  .filter(Boolean);

// 2) Usamos Set para quedarnos SOLO con valores únicos
const tagSet = new Set(tagsNormalizadas);

// Convertir a array cuando haga falta enviarlas/mostrarlas
const tagsUnicas = [...tagSet]; // ["javascript","css","html","node"]

// 3) Reusar el Set para validar nuevas etiquetas sin duplicar
function agregarTag(tag) {
  const t = tag.trim().toLowerCase();
  if (tagSet.has(t)) return { ok: false, msg: "La etiqueta ya existe" };
  tagSet.add(t);
  return { ok: true, msg: "Etiqueta agregada", total: tagSet.size };
}

agregarTag("CSS");        // { ok:false, msg:"La etiqueta ya existe" }
agregarTag("typescript"); // { ok:true,  msg:"Etiqueta agregada", total: 5 }

// 4) También puedes eliminar o vaciar:
tagSet.delete("html"); // true si la eliminó
// tagSet.clear();     // vacía todo
...
**Reto:** agrupa estudiantes por curso: `{ curso: 'A' | 'B' }`.
...
const alumnos = [
  { nombre: "Ana", curso: "A" },
  { nombre: "Luis", curso: "B" },
  { nombre: "Sofía", curso: "A" },
];

const grupos = new Map();
for (const a of alumnos) {
  if (!grupos.has(a.curso)) grupos.set(a.curso, []);
  grupos.get(a.curso).push(a);
}
// Map { "A" => [Ana, Sofía], "B" => [Luis] }
```

## Merge de conteos (sumar valores por clave)

**Reto:** combina ventas de dos sucursales sumando por producto.

```
const s1 = new Map([["café", 10], ["té", 5]]);
const s2 = new Map([["café", 7], ["mate", 3]]);
```
...
const total = new Map(s1);
for (const [k, v] of s2) {
  total.set(k, (total.get(k) || 0) + v);
}
// Map { "café" => 17, "té" => 5, "mate" => 3 }
```
...
# Metodos para manipular datos en estructuras de datos

<img src="https://i.ibb.co/cSmV0wm5/image.png" style="zoom:50%;" />
...
Se usa cuando cada elemento necesita transformarse y devolverse en **otro arreglo del mismo tamaño**. No altera el original.
...
- Firma del callback: `(element, index, array)`
- Caso típico: formatear datos, calcular valores, reemplazar símbolos.

------

## `filter(callback)`

Sirve para obtener **solo los elementos que cumplen una condición**, dejando fuera el resto. Tampoco muta.

```
const soloTazas = arr.filter(x => x === '🍵');
// ['🍵','🍵','🍵']
```

- Caso típico: crear subconjuntos (activos, aprobados, mayores de X, etc.).

------

## `find(callback)`

Entrega **el primer elemento** que cumpla la condición, o `undefined` si no aparece. No muta.

```
const primeraTaza = arr.find(x => x === '🍵'); // '🍵'
```

- Caso típico: recuperar **un** registro concreto (el primero que calce).

------

## `findIndex(callback)` vs `indexOf(valor)`

Ambos devuelven **índices**, pero con reglas distintas.

```
const idxConCondicion = arr.findIndex(x => x === '🍵'); // 0
const idxPorIgualdad  = arr.indexOf('🥤');              // 2
```

- `findIndex` acepta **condiciones complejas**.
- `indexOf` compara por **igualdad estricta (`===`)** un valor **conocido** (útil con primitivos).

------

## `fill(value, start = 0, end = arr.length)`

Rellena con un valor desde `start` hasta `end` **y sí muta** el arreglo.

```
const copia = [...arr];
copia.fill('🥤', 1);
// ['🍵','🥤','🥤','🥤','🥤']  ← 'copia' quedó modificada
```

- Uso típico: inicializar rangos, resetear secciones, preparar buffers.

------

## `some(callback)`

Responde `true` si **al menos un** elemento cumple la condición. Corta en cuanto lo encuentra. No muta.

```
const hayTaza = arr.some(x => x === '🍵'); // true
```

- Uso típico: validaciones tipo “existe alguno…”.

------

## `every(callback)`

Responde `true` solo si **todos** los elementos cumplen la condición. Corta en el primer incumplimiento. No muta.

```
const todasTazas = arr.every(x => x === '🍵'); // false
```

- Uso típico: reglas tipo “todo el conjunto debe…”.

------

## Detalles que suelen marcar la diferencia

- Los callbacks reciben `(elem, i, array)`; conviene aprovechar el índice cuando la posición importa.
- `map`, `filter`, `find`, `findIndex`, `some`, `every` **no** mutan; `fill` **sí**.
- `indexOf` usa `===`; con objetos y `NaN` no se comporta como muchos esperan.
- `some`/`every` y `find`/`findIndex` **terminan temprano**, lo que ahorra recorrido en listas grandes.
- Cuando el interés está en el **último** que cumpla, ES2023 añade `findLast` y `findLastIndex`.
...
2. cree una carpeta llamada data y dentro de data cree un archivo llamado heroes.js y pegue el contenido del gist de recursos
...
Un **callback** es una función que se entrega como argumento a otra función para que esta la **ejecute después**, cuando sea necesario.
...
boton.addEventListener('click', function () {
  console.log('Haz clic para ver que funciona')
})
```
...
console.log('Fin: Script cargado, esperando acciones del usuario')
```
...
`textContent` es **una propiedad de cualquier nodo del DOM** que:

- **Lee** todo el texto del nodo y sus descendientes (sin etiquetas HTML).
- **Escribe** texto reemplazando todos los hijos del nodo por un único nodo de texto.
- **No interpreta HTML** (si asignas `<b>hola</b>`, se verá como texto literal).
- Incluye texto aunque el elemento esté oculto con CSS (`display: none`).
- Úsala para **insertar texto de usuario de forma segura** (evita XSS), o cuando no necesitas dar formato HTML.

```javascript
const h1 = document.querySelector('h1');
console.log(h1.textContent);       // Lee el texto
h1.textContent = 'Título seguro';  // Reemplaza todo por texto plano
```
...
> Diferencias rápidas:
>
> - `innerHTML` **sí** interpreta/inyecta HTML (útil pero más riesgoso).
> - `innerText` respeta el **layout/estilos** (no incluye texto oculto) y puede forzar recálculo de estilo; suele ser más lento.

## ¿Qué es `getBoundingClientRect()`?

`getBoundingClientRect()` es **un método de los elementos del DOM** que devuelve un **`DOMRect`** con **posición y tamaño** del elemento **respecto al viewport** (ventana visible) en **píxeles CSS**.

Devuelve, entre otras, estas propiedades:

- `x`, `y` (alias de `left`, `top`)
- `left`, `top`, `right`, `bottom`
- `width`, `height`

Notas clave:

- Los valores pueden ser **decimales** (subpíxel).
- **Incluye transformaciones CSS** (ej. `transform: scale(...)`).
- Para convertir a coordenadas del **documento completo**, suma el scroll: `window.scrollX`/`window.scrollY`.
...
console.log(rect.width, rect.height); // tamaño visible
console.log(rect.left, rect.top);     // posición relativa al viewport
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="js/dom02.js" defer></script>
    <link rel="stylesheet" href="css/dom02.css">
    <title>Document</title>
</head>
<body>
    <button id="boton-importante">Haz clic</button>
    <button id="boton-demo">boton dos</button>
</body>
</html>
```
...
```css
  ...
  /* Estado aplicado */
  #boton-importante.aplicado {
    background-color: #22c55e; /* igual al ejemplo */
    color: #fff;
    border: none;
    cursor: not-allowed;
    filter: saturate(1.1);
...
### 1) Deshabilitar un botón al hacer clic
...
<script>
  ...
  btn.addEventListener('click', () => {
    btn.disabled = true;          // propiedad booleana
    btn.textContent = 'Enviando…';
  });
</script>
```

### 2) Alternar habilitado/deshabilitado con un checkbox
...
<button id="action">Acción</button>

<script>
  const chk = document.querySelector('#toggle');
  const action = document.querySelector('#action');

  function sync() { action.disabled = !chk.checked; }
  chk.addEventListener('change', sync);
  sync(); // estado inicial
</script>
```

### 3) Deshabilitar un grupo completo con `<fieldset>`
...
<button id="lock">Bloquear/Desbloquear</button>

<script>
  const grupo = document.querySelector('#grupo');
  document.querySelector('#lock').addEventListener('click', () => {
    grupo.disabled = !grupo.disabled; // deshabilita todo el fieldset
  });
</script>
```

### 4) Mientras se hace una operación asíncrona
...
<script>
  ...
  save.addEventListener('click', async () => {
    try {
      ...
      save.textContent = 'Listo';
    } catch (e) {
      save.textContent = 'Reintentar';
    } finally {
      save.disabled = false; // o déjalo true para evitar reintentos
    }
  });
</script>
```
...
### 1) Leer y escribir HTML (con etiquetas)

```html
<div id="box"><b>Hola</b> mundo</div>
<script>
  const box = document.querySelector('#box');
  console.log(box.innerHTML);        // "<b>Hola</b> mundo"

  box.innerHTML = '<i>Nuevo</i> <u>contenido</u>'; // interpreta HTML
</script>
```

### 2) Insertar sin reemplazar todo (más eficiente)

```javascript
const lista = document.querySelector('#items');
// Inserta al final sin tocar lo demás
lista.insertAdjacentHTML('beforeend', '<li><strong>Nuevo</strong> ítem</li>');
```

### 3) Peligro XSS: NO metas entrada de usuario en `innerHTML`

> [!WARNING]
>
> **XSS (Cross-Site Scripting)** es una vulnerabilidad que permite a un atacante **inyectar y ejecutar JavaScript malicioso en el navegador** de otra persona usando tu página. Con eso puede robar sesiones/tokens, leer o modificar el DOM, redirigir, minar datos, etc.

```javascript
// ❌ PELIGRO: 'name' viene del usuario
saludo.innerHTML = `<p>Hola, ${name}</p>`; // puede inyectar HTML/script

// ✅ SEGURO: usa textContent o crea nodos
saludo.textContent = `Hola, ${name}`;
```

### 4) Crear estructura compleja sin `innerHTML` (seguro)
...
p.textContent = 'Texto seguro sin HTML interpretado.';
card.replaceChildren(title, p);
```

## Diferencias clave

- **`textContent`**: trata todo como **texto plano** (no HTML). Seguro para datos de usuario.
- **`innerText`**: similar a texto visible (respeta estilos/ocultos) y puede forzar reflow.
- **`outerHTML`**: igual que `innerHTML` pero incluye **el propio elemento**.

## ¿Qué es `outerHTML`?

`outerHTML` es una **propiedad de los elementos del DOM** que:

- **Lee** el HTML **del elemento completo**, **incluyéndose a sí mismo** (etiqueta de apertura, contenido y cierre).
- **Escribe** HTML **reemplazando** el elemento actual en el DOM por el nuevo marcado que asignes.

## Ejemplos rápidos

**1) Leer el HTML completo de un elemento**

```html
<li class="item">Texto</li>
<script>
  const li = document.querySelector('.item');
  console.log(li.outerHTML); // "<li class="item">Texto</li>"
</script>
```

**2) Reemplazar un elemento por otro usando `outerHTML`**

```javascript
const li = document.querySelector('.item');
li.outerHTML = '<div class="card">Contenido</div>'; // el <li> desaparece y queda <div>
```

**3) Envolver un elemento (wrap)**

```javascript
const btn = document.querySelector('#pagar');
btn.outerHTML = `<div class="wrapper">${btn.outerHTML}</div>`;
// Nota: la referencia previa a btn ya no es válida; el DOM lo reemplazó.
```

## Diferencias clave

| Propiedad     | ¿Incluye al propio elemento? | ¿Interpreta HTML al asignar? | Uso típico                        |
| ------------- | ---------------------------- | ---------------------------- | --------------------------------- |
| `textContent` | No aplica (solo texto)       | No                           | Texto plano seguro (evita XSS)    |
| `innerHTML`   | ❌ No                         | ✔️ Sí                         | Cambiar **contenido interno**     |
| `outerHTML`   | ✔️ Sí                         | ✔️ Sí                         | **Reemplazar** el elemento entero |

## Advertencias importantes

- **Reemplaza el nodo**: la referencia JS al elemento antiguo **queda inválida** (debes buscar el nuevo).

- **No conserva listeners**: al reemplazar, **pierdes** los `addEventListener` de ese nodo/hijos.

- **XSS**: jamás insertes contenido de usuario sin sanear (`outerHTML` e `innerHTML` **interpretan** HTML).

- **Rendimiento**: para cambios estructurados, suele ser más claro/seguro usar APIs del DOM:

  ```
  const nuevo = document.createElement('div');
  nuevo.className = 'card';
  nuevo.textContent = 'Contenido';
  elemento.replaceWith(nuevo); // alternativa segura a outerHTML
  ```
